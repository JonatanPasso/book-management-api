...
class ReportAuthorService
{
     ...
     * @throws LoaderError
     */
    public function generateReportByAuthor(Request $request): Response
    {
        $books = $this->bookByAuthorAndSubjectRepository->findAllGroupedByAuthor();

        $html = $this->twig->render('report/author.html.twig', [
            'books' => $books
        ]);
        ...
        if ($isDownload) {
            return new Response(
                ...
                [
                    ...
                    'Content-Disposition' => 'attachment; filename="relatorio_por_autor.pdf"' // Forçar o download
                ]
            );
        }
